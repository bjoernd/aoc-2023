        ...
        for (i, l) in lines.enumerate() {
            if i == 0 {
                ...
                for (j, v) in raw.split_whitespace().enumerate() {
                    match j {
                        0 => { source = String::from(v); }
                        1 => { dest_l = String::from(v); }
                        2 => { dest_r = String::from(v); }
                        _ => { }
                    }
                }

                connections.insert(source, Target{ left: dest_l, right: dest_r });
            }
        }
        Day8{ targets: connections, directions }
    }
}
    ...
    fn part_one(&self) -> String {
        ...
        while current_node.as_str() != "ZZZ" {
            ...
            match next_direction {
                'L' => { current_node = self.targets[&current_node].left.clone(); },
                'R' => { current_node = self.targets[&current_node].right.clone(); },
                _ => {}
            }
            steps += 1;
        }
